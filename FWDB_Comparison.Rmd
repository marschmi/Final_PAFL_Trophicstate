...
```{r data import of Original Mothur data classified with Silva DB only}
### Data import
...
scaled_historgram <- ggplot(nowin_data, aes(x = TotalSeqs_Scaled, fill = filter)) + ylab("Count") +
  ...
  #geom_histogram(binwidth=20, alpha=.75, position="identity") +
  #geom_density(alpha=.5) + 
  xlab("Total Sequences per Sample") + #fill="thistle1",
...
```{r Figure S3:  sum and scaled Alpha diversity, fig.width = 12, fig.height = 15}
...
Total_reads <- c(sum(sum(part_only$OTU_sum_Particle) + sum(pvf_both$OTU_sum_Particle)), sum(sum(part_only$OTU_sum_Particle) + sum(pvf_both$OTU_sum_Particle)), 
                 sum(sum(free_only$OTU_sum_Free) + sum(pvf_both$OTU_sum_Free)), sum(sum(free_only$OTU_sum_Free) + sum(pvf_both$OTU_sum_Free)),
                 sum(sum(epi_only$OTU_sum_Epi) + sum(tvb_both$OTU_sum_Epi)), sum(sum(epi_only$OTU_sum_Epi) + sum(tvb_both$OTU_sum_Epi)),
                 sum(sum(hypo_only$OTU_sum_Hypo) + sum(tvb_both$OTU_sum_Hypo)),  sum(sum(hypo_only$OTU_sum_Hypo) + sum(tvb_both$OTU_sum_Hypo)),
                 sum(sum(high_only$OTU_sum_High) + sum(hvl_both$OTU_sum_High)),  sum(sum(high_only$OTU_sum_High) + sum(hvl_both$OTU_sum_High)),
                 sum(sum(low_only$OTU_sum_Low) + sum(hvl_both$OTU_sum_Low)),  sum(sum(low_only$OTU_sum_Low) + sum(hvl_both$OTU_sum_Low)))
...
###  PROPORTION OF OTUS 
proportion_PAFL_chisq <-matrix(c(otu_proportions[1], otu_proportions[2], otu_proportions[3], otu_proportions[4]), nrow=2)
proportion_PAFL_chisq_res <- chisq.test(proportion_PAFL_chisq, correct = FALSE); proportion_PAFL_chisq_res

fisher.test(proportion_PAFL_chisq)

proportion_prod_chisq <-matrix(c(otu_proportions[5], otu_proportions[6], otu_proportions[7], otu_proportions[8]),nrow=2)
proportion_prod_chisq_res <- chisq.test(proportion_prod_chisq, correct = FALSE); proportion_prod_chisq_res

proportion_limnion_chisq <-matrix(c(otu_proportions[9], otu_proportions[10], otu_proportions[11], otu_proportions[12]),nrow=2)
proportion_limnion_chisq_res <- chisq.test(proportion_limnion_chisq, correct = FALSE); proportion_limnion_chisq_res

#proportion_shared_chisq <- matrix(c(otu_proportions[1], otu_proportions[2], otu_proportions[3], otu_proportions[4]),nrow=1)
#proportion_shared_chisq_res <- chisq.test(proportion_shared_chisq, correct = FALSE); shared_chisq_res
...
otu_sum_plot <- ggplot(otu_overlap, aes(y=NumOTUs , x=SampleType, fill=Type, order=Type)) +
  ...
  scale_x_discrete(breaks=c("Free-Living", "Particle-Associated", "Low-Nutrient", "High-Nutrient", "Epilimnion", "Hypolimnion"),
                      labels=c("Free-Living (21)", "Particle-Associated (20)", "Low-Nutrient (15)", "High-Nutrient (26)", "Epilimnion (19)", "Hypolimnion (18)")) + 
...
nmds_bc_quad <- ggplot(nmds_bray, aes(MDS1*100, MDS2*-100, color = filter, shape = limnion, fill = ProdLevel)) +
  ...
  annotate("text", label = "Stress = 0.17", x = (max(nmds_bray$MDS1*100)-2.5), y = (max(nmds_bray$MDS2*100) + 0.1), size = 4, colour = "black") +
...
nmds_soren_quad <- ggplot(nmds_soren, aes(MDS1, MDS2, color = filter, shape = limnion, fill = ProdLevel)) +
  ...
  scale_shape_manual(name = "Lake Strata", breaks = c("Epilimnion", "Hypolimnion", "Mixed"), 
                     values = c(21, 22, 24)) +
  #scale_size_manual(name = "Nutrient Level", breaks = c("Productive", "Unproductive"), 
  #                   labels = c("High-Nutrient", "Low-Nutrient"),
  #                   values = c(6, 4)) +
...
multiplot(nmds_soren_quad, nmds_soren_quad_limnion, cols = 2)
...
# #Run an ADONIS test!
adonis_PAFL_mult <- adonis(BCdist ~ limnion+filter +ProdLevel+ DO+temp + pH, data=environ); adonis_PAFL_mult # R2 = 0.36245
...
part_adon_mult<- adonis(part_BC ~ limnion+ProdLevel+ DO + temp + pH, data=part_env); part_adon_mult #R2 = 
...
free_adon_mult<- adonis(free_BC ~ limnion+ProdLevel+ DO + temp + pH, data=free_env); free_adon_mult #R2 = 
...
epi_adon_mult<- adonis(epi_BC ~ filter+ProdLevel+ DO + temp + pH, data=epi_env); epi_adon_mult#R2 = 
...
hypo_adon_mult <- adonis(hypo_BC ~ filter+ProdLevel+DO + temp + pH, data=hypo_env); hypo_adon_mult # R2 = 
...
oligo_adon_mult <- adonis(oligo_BC ~ limnion+filter+DO + temp + pH, data=oligo_env); oligo_adon_mult # R2 = 
...
prod_adon_mult <- adonis(prod_BC ~ limnion+filter+DO + temp + pH, data=prod_env); prod_adon_mult # R2 = 
...
bray_boxplot <- ggplot(bray_try, aes(x = troph_lim1, y = value, fill = troph_lim1)) + geom_point(size = 2) +
 ...
 # geom_text(aes(label = siglabel, x = troph_lim1, y = 0.92), size =3) +
  theme(plot.title = element_text(face="bold", size = 12),  #Set the plot title
                                 ...
                                 plot.margin = unit(c(0.1, 0.1, 0.1, 0.1), "cm")); bray_boxplot #top, right, bottom, left
...
norm_soren  # soren Curtis dissimiliarity matrix for McMurdie & Holmes scaled, NO HYPO WINTERGREEN, samples
...
soren_boxplot <- ggplot(soren_try, aes(x = troph_lim1, y = value, fill = troph_lim1)) + geom_point(size = 2) +
  #geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.2, position=position_dodge(.9)) +
  geom_boxplot() +
  geom_text(data = soren_sigs, aes(label = siglabel, x = troph_lim1, y = 0.73), size =3) +
  scale_fill_manual(name = "", limits=c("Productive Epilimnion Particle", "Productive Epilimnion Free", "Productive Hypolimnion Particle", "Productive Hypolimnion Free",
                                         "Unproductive Epilimnion Particle", "Unproductive Epilimnion Free", "Unproductive Hypolimnion Particle", "Unproductive Hypolimnion Free"), 
                     values = c("royalblue4", "royalblue4", "royalblue4", "royalblue4",
                                "cornflowerblue","cornflowerblue","cornflowerblue","cornflowerblue"))+
  ...
  theme(plot.title = element_text(face="bold", size = 12),  #Set the plot title
                                 ...
                                 legend.text = element_text(size = 8),  #Set the legend text
                                 legend.position="none", #Default the legend position to the right
                                 plot.margin = unit(c(0.1, 0.1, 0.1, 0.1), "cm"));  soren_boxplot #top, right, bottom, left
```



```{r Fig 3 drawing plot}
...
DESeq_prodLevel_plot <- plot_phylum_deSEQ(DESeq_prodLevel, "Productivity Level")


###########################################################  Epilimnion vs Hypolimnion
DESeq_limnion <- deSEQ(good_phylum_nosherwin, ~ limnion)
DESeq_limnion_plot <- plot_phylum_deSEQ(DESeq_limnion, "Lake Layer")


###########################################################  Particle vs Free
DESeq_filter <- deSEQ(good_phylum_nosherwin, ~ filter)
DESeq_filter_plot <- plot_phylum_deSEQ(DESeq_filter, "Filter Fraction")


DESeq_habitats <- multiplot(DESeq_prodLevel_plot + theme(legend.position = "none"), 
                            DESeq_limnion_plot +theme(legend.position = "none"),
                            DESeq_filter_plot + theme(legend.position = "none"), cols = 3)
...
g3 <- ggplot(limnion_phy_boxplot, aes(y=PercentAbund , x=Phylum, fill=limnion)) + #coord_cartesian(xlim = c(0, 30)) + 
  ggtitle("") + xlab("") + geom_boxplot() +  ylab("Mean Percent \nRelative Abundance") + xlab("Phylum") +
   scale_fill_manual(name = "Lake Strata", breaks=c("Epilimnion", "Hypolimnion"),
...
```{r Figure S6 - phylum level heat map}
...
ordered_otu_ratios$Genus <- factor(ordered_otu_ratios$Genus,levels = c("unclassified", "bacIV-B", "vadinBC27_wastewater-sludge_group", "bacII-A", "Paludibacter","bacIV-A",
                                                                       "bacIII-A", "bacI-A", "Emticicia", "bacVI-B", "Solitalea", "bacIII-B", "Synechococcus", "Anabaena", 
                                                                       "Snowella", "Planktothrix", "Pseudanabaena", "Luteolibacter", "CandidatusXiphinematobacter", "Pnec",
                                                                       "betIV-A", "betI-A", "Dechloromonas", "Iodobacter", "betVII-B", "Sterolibacterium", "Sideroxydans", 
                                                                       "Sulfuritalea", "PRD01a011B", "Candidatus_Branchiomonas", "betIII-A", "Nitrosospira", "acIV-A", "acIV-D", 
                                                                       "acTH1-A", "acI-A", "acI-B", "Myco", "acI-C", "Luna1-A", "acSTL-A", "Pirellula", "Candidatus_Nostocoida", 
                                                                       "CL500-3", "Planctomyces", "Candidatus_Anammoximicrobium", "Pir4_lineage", "Phenylobacterium", "Rickettsia", 
                                                                       "Azospirillum", "alfI-B", "alfII-A", "Haematobacter", "Magnetospirillum", "Rubellimicrobium", "Roseomonas",
                                                                       "Rhizomicrobium", "alfIV-A", "Roseospirillum", "OM27_clade", "Desulfomonile", "Peredibacter", "Desulfatirhabdium",
                                                                       "Syntrophus", "Desulfocapsa", "Sorangium", "Desulfobulbus", "Smithella", "Desulfobacula", "Phaselicystis",
                                                                       "Desulfovibrio", "Methylobacter", "Thiocystis", "Coxiella", "Crenothrix", "Thiodictyon", "Lamprocystis",
                                                                       "Candidatus_Competibacter", "Oscillochloris", "Chloronema", "Anaerolinea", "Leptolinea", "Victivallis", 
                                                                       "Chlorobium", "Armatimonas", "Incertae_Sedis", "Fastidiosipila", "Fonticella", "Clostridium_sensu_stricto_9",
                                                                       "Anaerofustis", "Geothrix", "Treponema", "Spirochaeta", "Leptospira", "Brevinema", "Sulfurospirillum", 
...
#####  Plotting FIGURE S7  #####  Plotting FIGURE S7  #####  Plotting FIGURE S7  #####  Plotting FIGURE S7  #####  Plotting FIGURE S7
...
#####  Plotting FIGURE S8  #####  Plotting FIGURE S8  #####  Plotting FIGURE S8  #####  Plotting FIGURE S8  #####  Plotting FIGURE S8
...
```{r Fig 5 right panel - OTU summed relative to its abundance - alpha = 0.01, cutoff = 0 }
...
sig_otus_relabund_0$Phylum <- factor(sig_otus_relabund_0$Phylum,levels = c("Bacteroidetes", "Cyanobacteria", "Verrucomicrobia", "Betaproteobacteria", "Actinobacteria", "Planctomycetes", "Alphaproteobacteria","unclassified", "Deltaproteobacteria",
                                               "Gammaproteobacteria", "Chloroflexi", "Lentisphaerae", "Armatimonadetes", "Firmicutes", "Chlorobi", "Acidobacteria", "Spirochaetae", "Candidate_division_OD1",
                                               "NPL-UPA2", "Deinococcus-Thermus", "Candidate_division_OP3", "Epsilonproteobacteria", "TM6", "TA18", "Chlamydiae", "Fibrobacteres", "Candidate_division_SR1", 
                                               "Candidate_division_BRC1", "BD1-5", "Candidate_division_WS3", "Tenericutes", "Elusimicrobia", "Gemmatimonadetes", "WCHB1-60","Fusobacteria", "Deferribacteres", 
                                               "Candidate_division_TM7", "Candidate_division_OP8", "SPOTSOCT00m83", "Thermotogae", "Candidate_division_OP11", "Dictyoglomi", "SM2F11", "Hyd24-12", "Nitrospirae",
                                               "SHA-109", "TA06", "OC31", "GOUTA4", "Caldiserica", "Candidate_division_JS1", "FGL7S"))
...
OTUtop16 <- c("Bacteroidetes", "Cyanobacteria", "Verrucomicrobia", "Betaproteobacteria", "Actinobacteria", "Planctomycetes", "Alphaproteobacteria", "unclassified","Deltaproteobacteria",
                                               "Gammaproteobacteria", "Chloroflexi", "Lentisphaerae", "Armatimonadetes", "Firmicutes", "Chlorobi", "Acidobacteria", "Spirochaetae")
...
```{r Fig 5 left panel - proportion of sig over total OTUs per phylum}
# PANEL A - 
...
otu_joined$Phylum <- factor(otu_joined$Phylum,levels = c("Bacteroidetes", "Cyanobacteria", "Verrucomicrobia", "Betaproteobacteria", "Actinobacteria", "Planctomycetes", "Alphaproteobacteria", "unclassified", "Deltaproteobacteria",
                                               "Gammaproteobacteria", "Chloroflexi", "Lentisphaerae", "Armatimonadetes", "Firmicutes", "Chlorobi", "Acidobacteria", "Spirochaetae", "Candidate_division_OD1",
                                               "NPL-UPA2", "Deinococcus-Thermus", "Candidate_division_OP3", "Epsilonproteobacteria", "TM6", "TA18", "Chlamydiae", "Fibrobacteres", "Candidate_division_SR1", 
                                               "Candidate_division_BRC1", "BD1-5", "Candidate_division_WS3", "Tenericutes", "Elusimicrobia", "Gemmatimonadetes", "WCHB1-60","Fusobacteria", "Deferribacteres", 
                                               "Candidate_division_TM7", "Candidate_division_OP8", "SPOTSOCT00m83", "Thermotogae", "Candidate_division_OP11", "Dictyoglomi", "SM2F11", "Hyd24-12", "Nitrospirae",
...
```{r Final Figure 5 Sig OTU plots}
...
```{r table 3 - top 5 sig OTUs in each habitat}

top5_sig_OTUs <- function(log2fold_data){
  log2fold_data %>%
  arrange(log2FoldChange) %>%
  select(Phylum, Genus, OTU, log2FoldChange, Habitat, Preference) %>%
  head(n=5)
}

low_top5 <- top5_sig_OTUs(pvu_low)

high_top5 <- top5_sig_OTUs(pvu_high)

top_top5 <- top5_sig_OTUs(tvb_top)
bottom_top5 <- top5_sig_OTUs(tvb_bottom)

free_top5 <- top5_sig_OTUs(pvf_free)
particle_top5 <- top5_sig_OTUs(pvf_particle)

top5_otus_combined <- rbind(free_top5, particle_top5, low_top5, high_top5, top_top5, bottom_top5)
top5_otus_combined$log2FoldChange <- abs(top5_otus_combined$log2FoldChange)
top5_otus_combined$Habitat <- as.character(top5_otus_combined$Habitat)
top5_otus_combined$Habitat[top5_otus_combined$Habitat == " Epilimnion \nLow-Nutrient"] <- "Epilimnion Low-Nutrient"
top5_otus_combined$Habitat[top5_otus_combined$Habitat == " Epilimnion \nHigh-Nutrient"] <- "Epilimnion High-Nutrient"
top5_otus_combined$Habitat[top5_otus_combined$Habitat == " Free-Living \nEpilimnion"] <- "Free-Living Epilimnion"
top5_otus_combined$Habitat[top5_otus_combined$Habitat == " Particle-Associated \n Hypolimnion"] <- "Particle-Associated Hypolimnion"
top5_otus_combined$Habitat[top5_otus_combined$Habitat == " Particle-Associated \n Epilimnion"] <- "Particle-Associated Epilimnion"
top5_otus_combined$Habitat[top5_otus_combined$Habitat == " Free-Living \nHypolimnion"] <- "Free-Living Hypolimnion"
top5_otus_combined$Habitat[top5_otus_combined$Habitat == " Low-Nutrient \n Free-Living"] <- "Low-Nutrient Free-Living"
top5_otus_combined$Habitat[top5_otus_combined$Habitat == " Low-Nutrient \n Particle-Associated"] <- "Low-Nutrient Particle-Associated"
top5_otus_combined$Habitat[top5_otus_combined$Habitat == " High-Nutrient \n Free-Living"] <- "High-Nutrient Free-Living"
top5_otus_combined$Habitat[top5_otus_combined$Habitat == " High-Nutrient \n Particle-Associated"] <- "High-Nutrient Particle-Associated"

colnames(top5_otus_combined)[5] <- "Specific Habitat"
colnames(top5_otus_combined)[5] <- "Habitat Preference"

write.csv(top5_otus_combined, file = "~/Final_PAFL_Trophicstate/FWDB_Figures/Table3_top5_SigOTUs.csv", row.names = FALSE)
```



```{r Nov 12 - OTU-level analysis cutoff-0-alpha-0.05}
####################################################  OTU LEVEL LOG-2-FOLD RATIO ANALYSIS  ####################################################  Working up to FIGURE 6 and Figure S5
####################################################  OTU LEVEL LOG-2-FOLD RATIO ANALYSIS  ####################################################  Working up to FIGURE 6 and Figure S5
####################################################  OTU LEVEL LOG-2-FOLD RATIO ANALYSIS  ####################################################  Working up to FIGURE 6 and Figure S5
### Subsetting Sherman lake for differences between particle and free living 
#sherm <- subset_samples(nowin_FWDB_scaled, lakenames == "Sherman")  
#shemeister_0.05 <- deSEQ(sherm, ~ filter, cutoff = 0, alpha = 0.05)
#shemeister_0.05 <- plot_deSEQ(shemeister_0.05, "Sherman: OTU-Level, 0.05")
#sherm_plots <- multiplot(shemeister_0.05,sherm_plot, cols = 2)
...
#1. Top prod --> Model fit not typical
topProd <- subset_samples(good_samps_nosherwin, ProdLevel == "Productive" & limnion == "Epilimnion")
de_topProd_0.05 <- deSEQ(topProd, ~ filter, cutoff = 0, alpha = 0.05) # fitType = "local"
topProd_plot_0.05 <- plot_deSEQ(de_topProd_0.05, "PA vs FL:  Surface Productive (OTU)")

#2 Top Oligo 
topUNProd <- subset_samples(good_samps_nosherwin, ProdLevel == "Unproductive" & limnion == "Epilimnion")
de_topUNProd_0.05 <- deSEQ(topUNProd, ~ filter, cutoff = 0, alpha = 0.05) # NOTHING SIGNIFICANT?!
topUNProd_plot_0.05 <- plot_deSEQ(de_topUNProd_0.05, "PA vs FL:  Surface Unproductive (OTU)")

#3 Bottom Productive 
botProd <- subset_samples(good_samps_nosherwin, ProdLevel == "Productive" & limnion == "Hypolimnion") 
de_botProd_0.05 <- deSEQ(botProd, ~ filter, cutoff = 0, alpha = 0.05)
botProd_plot_0.05 <- plot_deSEQ(de_botProd_0.05, "PA vs FL:  Bottom Productive (OTU)")

#4 Bottom Unproductive 
botUNProd <- subset_samples(good_samps_nosherwin, ProdLevel == "Unproductive" & limnion == "Hypolimnion") 
de_botUNProd_0.05 <- deSEQ(botUNProd, ~ filter, cutoff = 0, alpha = 0.05)
botUNProd_plot_0.05 <- plot_deSEQ(de_botUNProd_0.05, "PA vs FL:  Bottom Unproductive (OTU)")

#pafl_otu_0.05 <- multiplot(topProd_plot_0.05, botProd_plot_0.05, botUNProd_plot_0.05,cols = 2)
...
#1. Top prod 
prodPA <- subset_samples(good_samps_nosherwin, ProdLevel == "Productive" & filter == "Particle") 
de_prodPA_0.05 <- deSEQ(prodPA, ~ limnion, cutoff = 0, alpha = 0.05)
prodPA_plot_0.05 <- plot_deSEQ(de_prodPA_0.05, "Surface vs Bottom:  PA Productive (OTU)")

#2 Top Oligo 
unprodPA <- subset_samples(good_samps_nosherwin, ProdLevel == "Unproductive" & filter == "Particle") 
de_unprodPA_0.05 <- deSEQ(unprodPA, ~ limnion, cutoff = 0, alpha = 0.05)
unprodPA_plot_0.05 <- plot_deSEQ(de_unprodPA_0.05, "Surface vs Bottom:  PA Unproductive (OTU)")

#3 Bottom Productive 
prodFL <- subset_samples(good_samps_nosherwin, ProdLevel == "Productive" & filter == "Free") 
de_prodFL_0.05 <- deSEQ(prodFL, ~ limnion, cutoff = 0, alpha = 0.05)
prodFL_plot_0.05 <- plot_deSEQ(de_prodFL_0.05, "Surface vs Bottom:  FL Productive (OTU)")

#4 Bottom Unproductive 
unprodFL <- subset_samples(good_samps_nosherwin, ProdLevel == "Unproductive" & filter == "Free") 
de_unprodFL_0.05 <- deSEQ(unprodFL, ~ limnion, cutoff = 0, alpha = 0.05)
unprodFL_plot_0.05 <- plot_deSEQ(de_unprodFL_0.05, "Surface vs Bottom:  FL Unproductive (OTU)")

#topbot_otu_0.05 <- multiplot(prodPA_plot_0.05, prodFL_plot_0.05, unprodPA_plot_0.05, unprodFL_plot_0.05,cols = 2)

############################
###########################################################PROD VS OLIGO
#1. Top prod 
topPA <- subset_samples(good_samps_nosherwin, limnion == "Epilimnion" & filter == "Particle") 
de_topPA_0.05 <- deSEQ(topPA, ~ ProdLevel, cutoff = 0, alpha = 0.05)
topPA_plot_0.05 <- plot_deSEQ(de_topPA_0.05, "Productive vs Unproductive:  PA Surface (OTU)")

#2 Top Oligo 
botPA <- subset_samples(good_samps_nosherwin, limnion == "Hypolimnion" & filter == "Particle") 
de_botPA_0.05 <- deSEQ(botPA, ~ ProdLevel, cutoff = 0, alpha = 0.05)
botPA_plot_0.05 <- plot_deSEQ(de_botPA_0.05, "Productive vs Unproductive:  PA Bottom (OTU)")

#3 Bottom Productive 
topFL <- subset_samples(good_samps_nosherwin, limnion == "Epilimnion" & filter == "Free") 
de_topFL_0.05 <- deSEQ(topFL, ~ ProdLevel, cutoff = 0, alpha = 0.05)
topFL_plot_0.05 <- plot_deSEQ(de_topFL_0.05, "Productive vs Unproductive:  FL Surface (OTU)")

#4 Bottom Unproductive 
botFL <- subset_samples(good_samps_nosherwin, limnion == "Hypolimnion" & filter == "Free") 
de_botFL_0.05 <- deSEQ(botFL, ~ ProdLevel, cutoff = 0, alpha = 0.05)
botFL_plot_0.05 <- plot_deSEQ(de_botFL_0.05, "Productive vs Unproductive:  FL Bottom (OTU)")

#prodoligo_otu_0.05 <- multiplot(topFL_plot_0.05, botFL_plot_0.05, topPA_plot_0.05, botPA_plot_0.05, cols = 2)



#PA VS FL:  
#Surface Productive
pafl1_0.05 <- subset(de_topProd_0.05, select = c(Phylum, Genus, Species, OTU, log2FoldChange, padj))
pafl1_0.05$Habitat <- "PA vs. FL: Epilimnion \nHigh-Nutrient"
#Bottom High-Nutrient 
pafl2_0.05 <- subset(de_botProd_0.05, select = c(Phylum, Genus, Species, OTU, log2FoldChange, padj))
pafl2_0.05$Habitat <- "PA vs. FL: Epilimnion \nLow-Nutrient"
#Surface Low-Nutrient --> nothing significant!!!
#pafl3 <- subset(de_topUNProd, select = c(Phylum, Genus, Species,OTU, log2FoldChange, padj))
#pafl3$Habitat <- "PA vs. FL: Hypolimnion \nHigh-Nutrient"
#Bottom Low-Nutrient
pafl4_0.05 <- subset(de_botUNProd_0.05, select = c(Phylum, Genus, Species, OTU, log2FoldChange, padj))
pafl4_0.05$Habitat <- "PA vs. FL: Hypolimnion \nLow-Nutrient"



###Top vs Bottom:  
otu_topbot1_0.05 <- subset(de_prodPA_0.05, select = c(Phylum, Genus, Species, OTU, log2FoldChange, padj))
otu_topbot1_0.05$Habitat <- "Top vs. Bottom: High-Nutrient \n Particle-Associated"
# Particle-Associated Low-Nutrient
otu_topbot2_0.05 <- subset(de_unprodPA_0.05, select = c(Phylum, Genus, Species, OTU, log2FoldChange, padj))
otu_topbot2_0.05$Habitat <- "Top vs. Bottom: Low-Nutrient \n Particle-Associated"
# Free-Living High-Nutrient 
otu_topbot3_0.05 <- subset(de_prodFL_0.05, select = c(Phylum, Genus, Species, OTU, log2FoldChange, padj))
otu_topbot3_0.05$Habitat <- "Top vs. Bottom: High-Nutrient \n Free-Living"
# Free-Living Low-Nutrient
otu_topbot4_0.05 <- subset(de_unprodFL_0.05, select = c(Phylum, Genus, Species, OTU, log2FoldChange, padj))
otu_topbot4_0.05$Habitat <- "Top vs. Bottom: Low-Nutrient \n Free-Living"



#Prod vs Oligo:  
otu_troph1_0.05 <- subset(de_topPA_0.05, select = c(Phylum, Genus, Species, OTU, log2FoldChange, padj)) 
otu_troph1_0.05$Habitat <- "Prod vs. Unprod: Particle-Associated \n Epilimnion"
# Particle-Associated BOTTOM
otu_troph2_0.05 <- subset(de_botPA_0.05, select = c(Phylum, Genus, Species, OTU, log2FoldChange, padj))
otu_troph2_0.05$Habitat <- "Prod vs. Unprod: Particle-Associated \n Hypolimnion"
# Free-Living TOP 
otu_troph3_0.05 <- subset(de_topFL_0.05, select = c(Phylum, Genus, Species, OTU, log2FoldChange, padj))
otu_troph3_0.05$Habitat <- "Prod vs. Unprod: Free-Living \nEpilimnion"
# Free-Living BOTTOM
otu_troph4_0.05 <- subset(de_botFL_0.05, select = c(Phylum, Genus, Species, OTU, log2FoldChange, padj))
otu_troph4_0.05$Habitat <- "Prod vs. Unprod: Free-Living \nHypolimnion"


######  Combing all into one dataframe named out_ratio
otu_trophs_0.05 <- rbind(otu_troph1_0.05, otu_troph2_0.05, otu_troph3_0.05, otu_troph4_0.05)
newlog2foldchange_0.05 <- as.numeric(otu_trophs_0.05$log2FoldChange * -1) # To correlate with Top vs bottom
otu_trophs_0.05$log2FoldChange <- newlog2foldchange_0.05

otu_ratio_0.05 <-rbind(pafl1_0.05, pafl2_0.05, pafl4_0.05,
                  otu_topbot1_0.05, otu_topbot2_0.05, otu_topbot3_0.05, otu_topbot4_0.05,
                  otu_trophs_0.05) #otu_troph1, otu_troph2, otu_troph3, otu_troph4)

#What's the min and max ratios?
paste(c("The range of the log2foldChange is", min(otu_ratio_0.05$log2FoldChange), "to",  max(otu_ratio_0.05$log2FoldChange)))


#Split of the habitat column to 2 columns named comparison and habitat
otu_cols_05 <- colsplit(otu_ratio_0.05$Habitat, ":", c("Comparison", "Habitat"))
otu_ratio_0.05$Habitat = NULL
otus0_05 <- cbind(otu_ratio_0.05, otu_cols_05)
...
#head(otus0_05)
#subset out prod vs unprod
pvu0_05 <- subset(otus0_05, Comparison == "Prod vs. Unprod")
tvb0_05 <- subset(otus0_05, Comparison == "Top vs. Bottom")
pvf0_05 <- subset(otus0_05, Comparison == "PA vs. FL")
#multiplot(plot_deSEQ(pvu, "Prod vs. Unprod"), plot_deSEQ(tvb, "Top vs Bottom"), plot_deSEQ(pvf, "PA vs FL"), cols = 3)

####  High vs Low nutrient
pvu0_05$Preference <- "NA"
pvu_high0_05 <- filter(pvu0_05, log2FoldChange > 0)
pvu_high0_05$Preference <- "High-Nutrient" # High-Nutrient is POSITIVE  
pvu_low0_05 <- filter(pvu0_05, log2FoldChange < 0)
pvu_low0_05$Preference <- "Low-Nutrient" # Low-Nutrient is NEGATIVE 
pvu_pref0_05 <- rbind(pvu_low0_05, pvu_high0_05) %>% # Combine the PA and FL data frames
  group_by(Phylum, Preference) %>%  # group by preference and phylum
  tally() # count how many in each 
pvu_pref0_05$Comparison <- "Prod vs. Unprod"
# Add a new column where the Low nutrient values are negative  
pvu_pref0_05$Count <-  ifelse(pvu_pref0_05$Preference == "Low-Nutrient", pvu_pref0_05$n*-1, pvu_pref0_05$n)

  
#### Top (Epilimnion) vs Bottom (Hypolimnion) 
tvb0_05$Preference <- "NA"
tvb_bottom0_05 <- filter(tvb0_05, log2FoldChange > 0)
tvb_bottom0_05$Preference <- "Hypolimnion" # Hypolimnion is POSITIVE  
tvb_top0_05 <- filter(tvb0_05, log2FoldChange < 0)
tvb_top0_05$Preference <- "Epilimnion" # Epilimnion is NEGATIVE 
tvb_pref0_05 <- rbind(tvb_top0_05, tvb_bottom0_05) %>% # Combine the PA and FL data frames
  group_by(Phylum, Preference) %>%  # group by preference and phylum
  tally() # count how many in each 
tvb_pref0_05$Comparison <- "Top vs. Bottom"
#  Add a column where the Epilimnion values are negative in a new column name count
tvb_pref0_05$Count <-  ifelse(tvb_pref0_05$Preference == "Epilimnion", tvb_pref0_05$n*-1, tvb_pref0_05$n)

# Add a new column called count
  
###  Particle vs Free living 
pvf0_05$Preference <- "NA"
pvf_particle0_05 <- filter(pvf0_05, log2FoldChange > 0)
pvf_particle0_05$Preference <- "Particle-Associated"
pvf_free0_05 <- filter(pvf0_05, log2FoldChange < 0)
pvf_free0_05$Preference <- "Free-Living"
pvf_pref0_05 <- rbind(pvf_free0_05, pvf_particle0_05) %>% # Combine the PA and FL data frames
  group_by(Phylum, Preference) %>%  # group by preference and phylum
  tally() # count how many in each 
pvf_pref0_05$Comparison <- "PA vs. FL"
#  Add a new column called "count" where if the count is free-living we make it negative 
pvf_pref0_05$Count <-  ifelse(pvf_pref0_05$Preference == "Free-Living", pvf_pref0_05$n*-1, pvf_pref0_05$n)

  

#  Combine into one data frame 
otu_preference0_05 <- data.frame(rbind(pvf_pref0_05, pvu_pref0_05, tvb_pref0_05))


# Get the significant OTUs 
# summed relative to its abundance 
### Free-living
sigotu_free_unique0_05 <- left_join(pvf_free0_05, otu_abund_free, by = "OTU") %>%
  left_join(phy_abund_free, by = "Phylum") %>%
  select(Preference, Phylum, Genus, Species, OTU, Comparison, OTU_sum, phylum_sum) %>% # Select only important columns
  mutate(OTU_rel_abund_byphy = OTU_sum/phylum_sum) %>% # Calculate OTU relative abundance by phylum 
  distinct() %>% # Take only the rows that are unique (no repeat of OTUs)
...
# Particle-Associated 
sigotu_particle_unique0_05 <- left_join(pvf_particle0_05, otu_abund_particle, by = "OTU") %>%
  left_join(phy_abund_particle, by = "Phylum") %>%
  select(Preference, Phylum, Genus, Species, OTU, Comparison, OTU_sum, phylum_sum) %>% # Select only important columns
  mutate(OTU_rel_abund_byphy = OTU_sum/phylum_sum) %>% # Calculate OTU relative abundance by phylum 
  distinct() %>% # Take only the rows that are unique (no repeat of OTUs)
...
# Epilimnion 
sigotu_epilimnion_unique0_05 <- left_join(tvb_top0_05, otu_abund_epi, by = "OTU") %>%
  left_join(phy_abund_epi, by = "Phylum") %>%
  select(Preference, Phylum, Genus, Species, OTU, Comparison, OTU_sum, phylum_sum) %>% # Select only important columns
  mutate(OTU_rel_abund_byphy = OTU_sum/phylum_sum) %>% # Calculate OTU relative abundance by phylum 
  distinct() %>% # Take only the rows that are unique (no repeat of OTUs)
...
# Hypolimnion 
sigotu_hypolimnion_unique0_05 <- left_join(tvb_bottom0_05, otu_abund_hypo, by = "OTU") %>%
  left_join(phy_abund_hypo, by = "Phylum") %>%
  select(Preference, Phylum, Genus, Species, OTU, Comparison, OTU_sum, phylum_sum) %>% # Select only important columns
  mutate(OTU_rel_abund_byphy = OTU_sum/phylum_sum) %>% # Calculate OTU relative abundance by phylum 
  distinct() %>% # Take only the rows that are unique (no repeat of OTUs)
...
# High-Nutrient 
sigotu_high_nutrient_unique0_05 <- left_join(pvu_high0_05, otu_abund_high, by = "OTU") %>%
  left_join(phy_abund_high, by = "Phylum") %>%
  select(Preference, Phylum, Genus, Species, OTU, Comparison, OTU_sum, phylum_sum) %>% # Select only important columns
  mutate(OTU_rel_abund_byphy = OTU_sum/phylum_sum) %>% # Calculate OTU relative abundance by phylum 
  distinct() %>% # Take only the rows that are unique (no repeat of OTUs)
...
# Low-Nutrient 
sigotu_low_nutrient_unique0_05 <- left_join(pvu_low0_05, otu_abund_low, by = "OTU") %>%
  left_join(phy_abund_low, by = "Phylum") %>%
  select(Preference, Phylum, Genus, Species, OTU, Comparison, OTU_sum, phylum_sum) %>% # Select only important columns
  mutate(OTU_rel_abund_byphy = OTU_sum/phylum_sum) %>% # Calculate OTU relative abundance by phylum 
  distinct() %>% # Take only the rows that are unique (no repeat of OTUs)
...
dim(sigotu_free_unique0_05); dim(sigotu_particle_unique0_05); dim(sigotu_epilimnion_unique0_05); dim(sigotu_hypolimnion_unique0_05); dim(sigotu_low_nutrient_unique0_05); dim(sigotu_high_nutrient_unique0_05)


sig_otus_relabund0_05 <- rbind(sigotu_free_unique0_05, sigotu_particle_unique0_05, sigotu_epilimnion_unique0_05, sigotu_hypolimnion_unique0_05, sigotu_low_nutrient_unique0_05, sigotu_high_nutrient_unique0_05)


# Determine factor order  
sig_otus_relabund0_05$Preference <- factor(sig_otus_relabund0_05$Preference,
                                    levels = c("Particle-Associated","Free-Living","Low-Nutrient","High-Nutrient","Epilimnion", "Hypolimnion")) 

sig_otus_relabund0_05$Phylum <- factor(sig_otus_relabund0_05$Phylum,levels = c("Bacteroidetes", "Cyanobacteria", "Verrucomicrobia", "Betaproteobacteria", "Actinobacteria", "Planctomycetes", "Alphaproteobacteria",  "unclassified", "Deltaproteobacteria",
                                               "Gammaproteobacteria", "Chloroflexi", "Lentisphaerae", "Armatimonadetes", "Firmicutes", "Chlorobi", "Acidobacteria", "Spirochaetae", "Candidate_division_OD1",
                                               "NPL-UPA2", "Deinococcus-Thermus", "Candidate_division_OP3", "Epsilonproteobacteria", "TM6", "TA18", "Chlamydiae", "Fibrobacteres", "Candidate_division_SR1", 
                                               "Candidate_division_BRC1", "BD1-5", "Candidate_division_WS3", "Tenericutes", "Elusimicrobia", "Gemmatimonadetes", "WCHB1-60","Fusobacteria", "Deferribacteres", 
                                               "Candidate_division_TM7", "Candidate_division_OP8", "SPOTSOCT00m83", "Thermotogae", "Candidate_division_OP11", "Dictyoglomi", "SM2F11", "Hyd24-12", "Nitrospirae",
...
sig_otus_relabund0_05$Comparison <- factor(sig_otus_relabund0_05$Comparison,
                                    levels = c("PA vs. FL", "Prod vs. Unprod","Top vs. Bottom")) 


sig_otus_relabund0_05$Phylum = with(sig_otus_relabund0_05, factor(Phylum, levels = rev(levels(Phylum)))) # Reverse the order of the phyla so it plots logically 


#pdf(file="~/Final_PAFL_Trophicstate/FWDB_Figures/SigOTUs_ALL_Relabund.pdf",  width= 7.5, height=8)
sig_phy_relabund_plot0_05 <- ggplot(sig_otus_relabund0_05, aes(y=sig_phylum_relabund, x=Phylum, fill=Preference)) + 
  geom_bar(stat="identity", position="identity") + coord_flip() + #ggtitle("Summed OTUs") +
  geom_bar(stat="identity", colour = "black", show_guide = FALSE, position="identity") +
  theme_gray() +  #scale_y_continuous(breaks=seq(-30, 200, 30)) +
  ggtitle("All OTUs in the Data") +
  ...
  theme(axis.title.x = element_text(face="bold", size=12),
        ...
        plot.margin = unit(c(0.1, 0.1, 0.1, 0.1), "cm"), #top, right, bottom, left
        plot.title = element_text(size = 12, face = "bold", colour = "black"),
        legend.position="right"); sig_phy_relabund_plot0_05
#dev.off()
...
sigotu_relabund_160_05 <- sig_otus_relabund0_05[sig_otus_relabund0_05$Phylum %in% OTUtop16, ]
sigotu_relabund_160_05$Phylum = with(sigotu_relabund_160_05, factor(Phylum, levels = rev(levels(Phylum)))) # Reverse the order of the phyla so it plots logically 


#pdf(file="~/Final_PAFL_Trophicstate/FWDB_Figures/SigOTUs_Top16_Relabund.pdf",  width= 7.5, height=6)
sigotu_phy_relabund_plot0_05 <- ggplot(sigotu_relabund_160_05, aes(y=sig_phylum_relabund, x=Phylum, fill=Preference)) + 
  geom_bar(stat="identity", position="identity") + coord_flip() + #ggtitle("Summed OTUs") +
  geom_bar(stat="identity", colour = "black", show_guide = FALSE, position="identity") +
  theme_gray() +  #scale_y_continuous(breaks=seq(-30, 200, 30)) +
  #ggtitle("All OTUs in the Top 16 Phyla in the Data") +
  ...
  theme(axis.title.x = element_text(face="bold", size=12),
        ...
        plot.margin = unit(c(0.1, 0.1, 0.1, 0.1), "cm"), #top, right, bottom, left
        plot.title = element_text(size = 12, face = "bold", colour = "black"),
        legend.position="right"); sigotu_phy_relabund_plot0_05
#dev.off()
...
######################  PROPORTION PLOT 0.05 p-value cutoff 
######################  PROPORTION PLOT 0.05 p-value cutoff 
######################  PROPORTION PLOT 0.05 p-value cutoff 
######################  PROPORTION PLOT 0.05 p-value cutoff 
...
free_living_sig_otus0_05 <- pvf0_05 %>% # Make a data frame with significant OTUs unique to free living 
  filter(log2FoldChange < 0) %>%
  select(Phylum, Genus, Species, OTU, Comparison) %>%
  distinct() %>%
  group_by(Phylum) %>%
  tally() %>%
  mutate(Preference = "Free-Living", Comparison =  "PA vs. FL")
colnames(free_living_sig_otus0_05)[2] <- "SigOTU_Total"

particle_associated_sig_otus0_05 <- pvf0_05 %>% # Make a data frame with significant OTUs unique to free living 
  filter(log2FoldChange > 0) %>%
  select(Phylum, Genus, Species, OTU, Comparison) %>%
  distinct() %>%
  group_by(Phylum) %>%
  tally() %>%
  mutate(Preference = "Particle-Associated", Comparison =  "PA vs. FL")
colnames(particle_associated_sig_otus0_05)[2] <- "SigOTU_Total"

epilimnion_sig_otus0_05 <- tvb0_05 %>% # Make a data frame with significant OTUs unique to Epilimnion
  filter(log2FoldChange < 0) %>%
  select(Phylum, Genus, Species, OTU, Comparison) %>%
  distinct() %>%
  group_by(Phylum) %>%
  tally() %>%
  mutate(Preference = "Epilimnion", Comparison =  "Top vs. Bottom")
colnames(epilimnion_sig_otus0_05)[2] <- "SigOTU_Total"


hypolimnion_sig_otus0_05 <- tvb0_05 %>% # Make a data frame with significant OTUs unique to Hypolimnion
  filter(log2FoldChange > 0) %>%
  select(Phylum, Genus, Species, OTU, Comparison) %>%
  distinct() %>%
  group_by(Phylum) %>%
  tally() %>%
  mutate(Preference = "Hypolimnion", Comparison =  "Top vs. Bottom")
colnames(hypolimnion_sig_otus0_05)[2] <- "SigOTU_Total"


high_nutrient_sig_otus0_05 <- pvu0_05 %>% # Make a data frame with significant OTUs unique to high-nutrient
  filter(log2FoldChange > 0) %>%
  select(Phylum, Genus, Species, OTU, Comparison) %>%
  distinct() %>%
  group_by(Phylum) %>%
  tally() %>%
  mutate(Preference = "High-Nutrient", Comparison =  "Prod vs. Unprod")
colnames(high_nutrient_sig_otus0_05)[2] <- "SigOTU_Total"


low_nutrient_sig_otus0_05 <- pvu0_05 %>% # Make a data frame with significant OTUs unique to low-nutrient
  filter(log2FoldChange < 0) %>%
  select(Phylum, Genus, Species, OTU, Comparison) %>%
  distinct() %>%
  group_by(Phylum) %>%
  tally() %>%
  mutate(Preference = "Low-Nutrient", Comparison =  "Prod vs. Unprod")
colnames(low_nutrient_sig_otus0_05)[2] <- "SigOTU_Total"





#######  Combine the above data frames with the "all_otus" data frames calculated earlier in the file 

otu_join_free0_05 <- inner_join(free_living_sig_otus0_05, free_living_all_otus, by = "Phylum") %>%
  mutate(Proportion_sig2total = -1* (SigOTU_Total/AllOTU_Total))

otu_join_particle0_05 <- inner_join(particle_associated_sig_otus0_05, particle_associated_all_otus, by = "Phylum") %>%
  mutate(Proportion_sig2total = 1* (SigOTU_Total/AllOTU_Total))

otu_join_epi0_05 <- inner_join(epilimnion_sig_otus0_05, epilimnion_all_otus, by = "Phylum") %>%
  mutate(Proportion_sig2total = -1* (SigOTU_Total/AllOTU_Total))

otu_join_hypo0_05 <- inner_join(hypolimnion_sig_otus0_05, hypolimnion_all_otus, by = "Phylum") %>%
  mutate(Proportion_sig2total = 1* (SigOTU_Total/AllOTU_Total))

otu_join_high0_05<- inner_join(high_nutrient_sig_otus0_05, high_nutrient_all_otus, by = "Phylum") %>%
  mutate(Proportion_sig2total = 1* (SigOTU_Total/AllOTU_Total))

otu_join_low0_05 <- inner_join(low_nutrient_sig_otus0_05, low_nutrient_all_otus, by = "Phylum") %>%
  mutate(Proportion_sig2total = -1* (SigOTU_Total/AllOTU_Total))

otu_joined0_05 <- rbind(otu_join_free0_05, otu_join_particle0_05, otu_join_epi0_05, otu_join_hypo0_05, otu_join_high0_05, otu_join_low0_05)

otu_joined0_05$Preference <- factor(otu_joined0_05$Preference,
                                    levels = c("Particle-Associated","Free-Living","Low-Nutrient","High-Nutrient","Epilimnion", "Hypolimnion")) 

otu_joined0_05$Phylum <- factor(otu_joined0_05$Phylum,levels = c("Bacteroidetes", "Cyanobacteria", "Verrucomicrobia", "Betaproteobacteria", "Actinobacteria", "Planctomycetes", "Alphaproteobacteria",  "unclassified", "Deltaproteobacteria",
                                               "Gammaproteobacteria", "Chloroflexi", "Lentisphaerae", "Armatimonadetes", "Firmicutes", "Chlorobi", "Acidobacteria", "Spirochaetae", "Candidate_division_OD1",
                                               "NPL-UPA2", "Deinococcus-Thermus", "Candidate_division_OP3", "Epsilonproteobacteria", "TM6", "TA18", "Chlamydiae", "Fibrobacteres", "Candidate_division_SR1", 
                                               "Candidate_division_BRC1", "BD1-5", "Candidate_division_WS3", "Tenericutes", "Elusimicrobia", "Gemmatimonadetes", "WCHB1-60","Fusobacteria", "Deferribacteres", 
                                               "Candidate_division_TM7", "Candidate_division_OP8", "SPOTSOCT00m83", "Thermotogae", "Candidate_division_OP11", "Dictyoglomi", "SM2F11", "Hyd24-12", "Nitrospirae",
...
otu_joined0_05$Comparison <- factor(otu_joined0_05$Comparison, levels = c("PA vs. FL", "Prod vs. Unprod","Top vs. Bottom")) 
...
otu_joined_16_0_05 <- otu_joined0_05[otu_joined0_05$Phylum %in% OTUtop16, ]
otu_joined_16_0_05$Phylum = with(otu_joined_16_0_05, factor(Phylum, levels = rev(levels(Phylum)))) # Reverse the order of the phyla so it plots logically 



#pdf(file="~/Final_PAFL_Trophicstate/FWDB_Figures/Fig6_0.05_pval.pdf",  width= 7.5, height=6)
sigotu_proportion_plot0_05 <- ggplot(otu_joined_16_0_05, aes(y=Proportion_sig2total, x=Phylum, fill=Preference)) + 
  geom_bar(stat="identity", position="identity") + coord_flip() + #ggtitle("Summed OTUs") +
  geom_bar(stat="identity", colour = "black", show_guide = FALSE, position="identity") +
  theme_gray() +  scale_y_continuous(breaks=seq(-0.5, 1, 0.1), lim = c(-0.15, .35)) + 
  #ggtitle("All OTUs in the Top 16 Phyla in the Data") +
  facet_grid(. ~ Comparison, labeller = mf_labeller) + theme_bw() +
  #scale_y_continuous(breaks=seq(-0.3, 1, 0.1)) +
  ylab("Proportion of Significant to Total OTUs by Phylum") +  xlab("Phylum") + 
...
```{r attemt 2 fig 5 with 0.05 pval cutoff}
b1_0_05 <- sigotu_proportion_plot0_05 +   ggtitle("P-Value Cutoff of 0.05") + 
  theme(legend.position="none", plot.margin = unit(c(0.1, 0.05, 0.1, 0.1), "cm")) #top, right, bottom, left
b2_0_05 <- sigotu_phy_relabund_plot0_05 + xlab("") +
  ggtitle("P-Value Cutoff of 0.05") + 
  theme(axis.text.y = element_blank(), plot.margin = unit(c(0.1, 0.1, 0.1, 0.05), "cm")) #top, right, bottom, left

pdf(file="~/Final_PAFL_Trophicstate/FWDB_Figures/Fig6_pval_0.05.pdf",  width= 12, height=6)
grid.newpage()
pushViewport(viewport(layout=grid.layout(1,2,width=c(0.46,0.54))))
print(b1_0_05, vp=viewport(layout.pos.row=1,layout.pos.col=1))
print(b2_0_05, vp=viewport(layout.pos.row=1,layout.pos.col=2))
dev.off()
...
####  Looking at OTUs that are different between 0.01 and 0.05 analyses 
...
View(anti_join(tvb0_05, tvb, by = "OTU"))

View(anti_join(pvf0_05, pvf, by = "OTU"))
