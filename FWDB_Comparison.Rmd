...
```{r Create working phyloseq object}
##  Our phyloseq object!
...
pvf_0 <- rbind(pvf_free, pvf_particle)
pvu_0 <- rbind(pvu_low, pvu_high)
tvb_0 <- rbind(tvb_top, tvb_bottom)
...
phy_melt_free <- subset(phy_melt, filter == "Free") %>% # Subset out the free-living samples 
...
phy_melt_free$SampleType <- "Free"

#Particle-Associated - 20 Samples
phy_abund_part <- subset(phy_melt, filter == "Particle") %>% # Subset out the particle-associated samples 
  select(Sample, Abundance, Phylum) %>% # select only the important columns 
  group_by(Phylum) %>% # Combine all the phyla
  summarize(phylum_sum = sum(Abundance)) # Sum each phylum abundance count
phy_abund_part$SampleType <- "Particle"

# Epilimnion - 19 samples 
phy_abund_epi <- subset(phy_melt, limnion == "Epilimnion") %>% # Subset out the Epilimnion samples 
  select(Sample, Abundance, Phylum) %>% # select only the important columns 
  group_by(Phylum) %>% # Combine all the phyla
  summarize(phylum_sum = sum(Abundance)) # Sum each phylum abundance count
...
phy_abund_hypo <- subset(phy_melt, limnion == "Hypolimnion") %>% # Subset out the Hypolimnion samples 
  select(Sample, Abundance, Phylum) %>% # select only the important columns 
  group_by(Phylum) %>% # Combine all the phyla
  summarize(phylum_sum = sum(Abundance)) # Sum each phylum abundance count
...
phy_abund_high <- subset(phy_melt, ProdLevel == "Productive") %>% # Subset out the Productive samples 
  select(Sample, Abundance, Phylum) %>% # select only the important columns 
  group_by(Phylum) %>% # Combine all the phyla
  summarize(phylum_sum = sum(Abundance)) # Sum each phylum abundance count
...
phy_abund_low <- subset(phy_melt, ProdLevel == "Unproductive") %>% # Subset out the Unproductive samples 
  select(Sample, Abundance, Phylum) %>% # select only the important columns 
  group_by(Phylum) %>% # Combine all the phyla
  summarize(phylum_sum = sum(Abundance)) # Sum each phylum abundance count
...
## Free-Living - 21 samples
otu_melt_free <- subset(otu_melt, filter == "Free") %>% # Subset out the free-living samples 
  select(Sample, Abundance, otulum) %>% # select only the important columns 
  group_by(otulum) %>% # Combine all the otula
  summarize(otulum_sum = sum(Abundance)) # Sum each otulum abundance count
otu_melt_free$SampleType <- "Free"

#Particle-Associated - 20 Samples
otu_abund_part <- subset(otu_melt, filter == "Particle") %>% # Subset out the particle-associated samples 
  select(Sample, Abundance, otulum) %>% # select only the important columns 
  group_by(otulum) %>% # Combine all the otula
  summarize(otulum_sum = sum(Abundance)) # Sum each otulum abundance count
otu_abund_part$SampleType <- "Particle"

# Epilimnion - 19 samples 
otu_abund_epi <- subset(otu_melt, limnion == "Epilimnion") %>% # Subset out the Epilimnion samples 
  select(Sample, Abundance, otulum) %>% # select only the important columns 
  group_by(otulum) %>% # Combine all the otula
  summarize(otulum_sum = sum(Abundance)) # Sum each otulum abundance count
...
# Hypolimnion - 18 Samples 
otu_abund_hypo <- subset(otu_melt, limnion == "Hypolimnion") %>% # Subset out the Hypolimnion samples 
  select(Sample, Abundance, otulum) %>% # select only the important columns 
  group_by(otulum) %>% # Combine all the otula
  summarize(otulum_sum = sum(Abundance)) # Sum each otulum abundance count
...
# High-Nutrient - 26 Samples 
otu_abund_high <- subset(otu_melt, ProdLevel == "Productive") %>% # Subset out the Productive samples 
  select(Sample, Abundance, otulum) %>% # select only the important columns 
  group_by(otulum) %>% # Combine all the otula
  summarize(otulum_sum = sum(Abundance)) # Sum each otulum abundance count
...
# Low-Nutrient - 15 samples 
otu_abund_low <- subset(otu_melt, ProdLevel == "Unproductive") %>% # Subset out the Unproductive samples 
  select(Sample, Abundance, otulum) %>% # select only the important columns 
  group_by(otulum) %>% # Combine all the otula
  summarize(otulum_sum = sum(Abundance)) # Sum each otulum abundance count
...
```{r sig OTUs discovery}
all_otus <- data.frame(rbind(pvu_low, pvu_high, tvb_top, tvb_bottom, pvf_free, pvf_particle))
