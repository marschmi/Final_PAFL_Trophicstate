...
print(most_abundant_taxa)
FWDB_50_OTUs <- prune_taxa(names(most_abundant_taxa), nowin_FWDB_scaled)

FWDB_sums <- data.frame(taxa_sums(FWDB_50_OTUs))

ggplot(FWDB_sums,aes(x=row.names(FWDB_sums), y=taxa_sums.FWDB_50_OTUs.)) + ylab("Number of Sequences per OTU") + scale_x_discrete(expand = c(0,0)) + scale_y_continuous(expand = c(0,0)) + theme_classic() +
  geom_bar(stat="identity",colour="black",fill="darkturquoise")  + xlab("OTU Rank") + 
  ggtitle("FWDB+Silva DB: Rank Abundance Curve of Top 50 OTUs") + 
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank())
```
...
de_topProd <- deSEQ(topProd, ~ filter) # fitType = "local"
...
de_topUNProd <- deSEQ(topUNProd, ~ filter) # NOTHING SIGNIFICANT?!
...
otu_preference$Comparison <- factor(allOTU_results$Comparison,
