...
## Influence of Meaning between Replicate Samples 
...
##  Histogram of Sample Read Counts 
sum_scale_floor_plot <- ggplot(data.frame(sum=sample_sums(sum_scale)),aes(sum)) + ylab("Number of Sequences per Sample") +
  geom_histogram(colour="black",fill="plum2", binwidth = 20)  + xlab("Total Sequences") + 
...
## Influence of Productivity and Trophic State on Alpha Diversity 
```{r Alpha Diversity - rarefied, fig.height=10, fig.width=16}
...
```{r sum-scale-matround Alpha diversity}
...
```{r comparing alpha diversity methods, fig.width = 28, fig.height = 20}
